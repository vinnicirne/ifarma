...
export const MotoboyManagement = ({ profile }: { profile: any }) => {
    ...
    const handleSave = async () => {
        if (!formData.name || !formData.pharmacy_id || !formData.phone || !formData.password) {
            setStatus({ type: 'error', message: "Preencha os campos obrigatórios: Nome, Telefone, Senha e Farmácia." });
            return;
        }
    ...
    const confirmDelete = (id: string) => {
        setConfirmModal({
            ...
            description: 'Tem certeza que deseja remover este motoboy? Esta ação é irreversível.',
    ...
    const handleSaveEdit = async () => {
        if (!editData.name || !editData.phone) {
            setStatus({ type: 'error', message: 'Nome e Telefone obrigatórios' });
            return;
        }
        setSaving(true);
        try {
    ...
    const confirmToggleStatus = (moto: any) => {
        const newStatus = !moto.is_active;
        const action = newStatus ? 'DESBLOQUEAR' : 'BLOQUEAR';

        setConfirmModal({
            isOpen: true,
            title: `${action} Motoboy`,
            description: `Deseja realmente ${action.toLowerCase()} o acesso de ${moto.name}? O motoboy ${newStatus ? 'poderá' : 'não poderá mais'} acessar o aplicativo.`,
            confirmText: newStatus ? 'Desbloquear Acesso' : 'Bloquear Acesso',
            type: newStatus ? 'success' : 'danger',
            onConfirm: () => handleToggleStatus(moto, newStatus)
        });
    };
    ...
    const openContractModal = async (boy: any) => {
        if (!boy.pharmacy_id) {
            setStatus({ type: 'error', message: 'Este motoboy não está vinculado a nenhuma farmácia.' });
            return;
        }
    ...
    const handleSaveContract = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setStatus({ type: 'error', message: 'Erro ao salvar contrato: ' + error.message });
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="flex flex-col gap-6 relative">
            ...
            {status && (
                <div className={`fixed top-4 right-4 max-w-sm w-full p-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-slide-in z-[110] backdrop-blur-md border ${status.type === 'success'
                        ? 'bg-emerald-500/20 text-emerald-500 border-emerald-500/20'
                        : 'bg-red-500/20 text-red-500 border-red-500/20'
                    }`}>
                    {status.type === 'success' ? <CheckCircle size={20} /> : <AlertTriangle size={20} />}
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-70">Sistema</p>
                                        ...
                                        <tr key={moto.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                                            ...
                                            <td className="p-6">
                                                <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <button onClick={() => confirmToggleStatus(moto)} title={moto.is_active ? "Bloquear/Banir" : "Desbloquear"} className={`size-8 rounded-lg flex items-center justify-center transition-colors ${moto.is_active ? 'bg-slate-100 dark:bg-white/10 text-slate-400 hover:bg-red-500 hover:text-white' : 'bg-green-500 text-white hover:bg-green-600'}`}>
                                                        <MaterialIcon name={moto.is_active ? "block" : "check_circle"} className="text-base" />
                                                    </button>
                                                    <button onClick={() => confirmDelete(moto.id)} title="Excluir" className="size-8 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors">
