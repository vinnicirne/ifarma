...
-- SMART INDEX CREATION: Detects if column is 'timestamp' or 'created_at'
DO $$
DECLARE
    col_exists BOOLEAN;
BEGIN
    ...
    SELECT EXISTS (
        ...
        WHERE table_name = 'route_history' AND column_name = 'timestamp'
    ) INTO col_exists;
    ...
    ELSE
        ...
        SELECT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'route_history' AND column_name = 'created_at'
        ) INTO col_exists;
