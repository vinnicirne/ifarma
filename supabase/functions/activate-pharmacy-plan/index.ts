// ============================================================================
// EDGE FUNCTION: Activate Pharmacy Plan (Restored Logic + Fixes + PIX QR Retry)
// ============================================================================
...
async function fetchPixQrWithRetry(paymentId: string) {
  const MAX_ATTEMPTS = 12;          // ← aumentei ainda mais
  const INITIAL_DELAY = 1500;       // 1.5s inicial
  const DELAY_INCREMENT = 1000;     // 1s de incremento

  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    const qrRes = await asaasFetch(`/payments/${paymentId}/pixQrCode`);
    if (qrRes.ok && qrRes.data?.encodedImage) {   // ← exija que venha imagem
      console.log(`PIX QR obtido na tentativa ${attempt} após ${INITIAL_DELAY + (attempt - 1) * DELAY_INCREMENT}ms`);
      return {
        ok: true as const,
    ...
    console.warn(
      `[PIX QR] Tentativa ${attempt}/${MAX_ATTEMPTS} falhou para ${paymentId}: ` +
      `${qrRes.status} – ${qrRes.data?.encodedImage ? 'sem imagem' : 'sem resposta ok'}` 
    );
  ...
  return { ok: false as const, encodedImage: null, payload: null };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!authz.allowed) {
      console.error(
        `[activate-pharmacy-plan] Unauthorized. Reason: ${authz.reason}, Pharmacy: ${pharmacy_id}, User: ${authz.userId || "unknown"}`,
      );
      return json(
        {
          error: "Acesso negado",
          reason: authz.reason,
          details: `User: ${authz.userId || "None"}, Allowed: false`,
        },
        401,
      );
    }
    ...
    if (currentSub?.status === "active") {
      if (!plan_id || plan_id === currentSub.plan_id) {
        console.log(
          `[activate-pharmacy-plan] Subscription already active for pharmacy ${pharmacy_id}. Returning success.`,
        );
    ...
    console.log(
      `[activate-pharmacy-plan] Selected Plan: ${plan.name} (${targetPlanId}). Paid: ${isPaidPlan}`,
    );
    ...
    if (!isPaidPlan) {
      ...
      // Ensure Billing Cycle exists (non-blocking)
      try {
        ...
        await supabaseAdmin.from("billing_cycles").upsert(
          {
            pharmacy_id,
    ...
    if (!pharmacy.cnpj) {
      ...
      return json(
        { error: "CNPJ é obrigatório para ativar planos pagos. Atualize o cadastro da farmácia." },
        400,
      );
    }
    ...
    const customerPayload = {
      ...
      cpfCnpj: String(pharmacy.cnpj).replace(/\D/g, ""),
    ...
    if (!asaasCustomerId) {
      ...
      // Sync existing (não bloqueia se falhar)
      try {
        ...
        console.warn("[activate-pharmacy-plan] Asaas customer sync failed (non-blocking):", e);
      }
    }
    ...
    // 10. If existing PIX, try QR or self-heal
    if (pixPaymentId) {
      console.log(`[activate-pharmacy-plan] Checking existing PIX: ${pixPaymentId}`);

      const qrTry = await fetchPixQrWithRetry(pixPaymentId);
      if (qrTry.ok) {
        pixQrBase64 = qrTry.encodedImage;
        pixCopyPaste = qrTry.payload;
      } else {
        ...
        if (status === "RECEIVED" || status === "CONFIRMED") {
          console.log(`[activate-pharmacy-plan] Payment ${pixPaymentId} is PAID. Updating DB.`);
        ...
        console.log(`[activate-pharmacy-plan] Existing PIX invalid/expired. Cancelling and recreating.`);
        await supabaseAdmin
          .from("billing_invoices")
    ...
    // 11. Create new PIX if needed
    if (!pixPaymentId) {
      console.log("[activate-pharmacy-plan] Generating PIX...");

      const value = Math.max((plan.monthly_fee_cents ?? 0) / 100, 5); // PIX mínimo R$5
      ...
      // ✅ Log detalhado após criar PIX
      console.log("[PIX criado]", {
        paymentId: pixPaymentId,
        status: pixRes.data.status,
        netValue: pixRes.data.netValue,
        invoiceUrl: pixRes.data.invoiceUrl,
        confirmedDate: pixRes.data.confirmedDate,
      });
      ...
      const { error: invErr } = await supabaseAdmin.from("billing_invoices").insert({
        ...
        amount_cents: Math.max(plan.monthly_fee_cents ?? 0, 500),
        due_date: firstPaymentDue,
        status: "pending",
        asaas_invoice_url: pixInvoiceUrl,
      ...
      // ✅ NEW: try to fetch QR right after creating PIX
      const qrTry = await fetchPixQrWithRetry(pixPaymentId);
      if (qrTry.ok) {
        pixQrBase64 = qrTry.encodedImage;
        pixCopyPaste = qrTry.payload;
    ...
    // 12. Subscription Spec (Idempotent)
    ...
    if (!asaasSubscriptionId) {
      ...
      const subRes = await asaasFetch("/subscriptions", {
        ...
        body: JSON.stringify({
          customer: asaasCustomerId,
          billingType: "BOLETO",
          value: Math.max((plan.monthly_fee_cents ?? 0) / 100, 5),
          cycle: "MONTHLY",
    ...
    // 13. Upsert Subscription (Pending Payment)
    ...
    const pixObj = pixPaymentId
      ? {
          ...
          invoice_url: pixInvoiceUrl || null,
          qr_base64: pixQrBase64 || null, // can be null if pending
          copy_paste: pixCopyPaste || null, // can be null if pending
          status: pixQrBase64 ? "ready" : "pending_qr",
        }
      : null;

    console.log("[activate-pharmacy-plan] Retornando pixObj:", JSON.stringify(pixObj, null, 2));
