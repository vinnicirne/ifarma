...
serve(async (req) => {
    ...
    try {
        ...
        if (email && password && password.length >= 6) {
            ...
            if (createError) {
                // If user already exists, we don't fail the approval but log it
                if (createError.message.includes("already registered") || createError.message.includes("already exists")) {
                    console.log(`[create-user-admin] User ${email} already exists.`);
                    // Try to find the existing user to return it
